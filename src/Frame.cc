...
//Copy Constructor
    ...
    Frame::Frame(const cv::Mat &imGray, const double &timeStamp, ORBextractor *extractor, ORBVocabulary *voc,
                 cv::Mat &K, cv::Mat &distCoef, const float &bf, const float &thDepth,flightdata fd)
            : mpORBvocabulary(voc), mpORBextractorLeft(extractor),
              mpORBextractorRight(static_cast<ORBextractor *>(NULL)),
              mTimeStamp(timeStamp), mK(K.clone()), mDistCoef(distCoef.clone()), mbf(bf), mThDepth(thDepth) ,mstFD(fd){
        // Frame ID
        mnId = nNextId++;
        ...
        // ORB extraction
        ExtractORBwithFD(0, imGray,mstFD);
    ...
    void Frame::ExtractORBwithFD(int flag, const cv::Mat &im,flightdata fd) {
        if (flag == 0)
            (*mpORBextractorLeft)(im, fd, mvKeys, mDescriptors);
        else
            (*mpORBextractorRight)(im, fd, mvKeysRight, mDescriptorsRight);

    }
/*
    void Frame::ExtractORB(int flag, const cv::Mat &im, double k, double b) {
        if (flag == 0)
            (*mpORBextractorLeft)(im, cv::Mat(), mvKeys, mDescriptors);
        else
            (*mpORBextractorRight)(im, cv::Mat(), mvKeysRight, mDescriptorsRight);
